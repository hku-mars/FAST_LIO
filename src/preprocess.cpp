...
Preprocess::Preprocess()
        : feature_enabled(0), lidar_type(AVIA), blind(0.01), point_filter_num(1), multi_echo_num(0) {
    inf_bound = 10;
    N_SCANS = 6;
    SCAN_RATE = 10;
    group_size = 8;
...
//                ((msg->points[i].tag & 0x30) == 0x20 || (msg->points[i].tag & 0x30) == 0x30 || (msg->points[i].tag & 0x30) == 0x00)){
//                double dis = sqrt(msg->points[i].x * msg->points[i].x + msg->points[i].y * msg->points[i].y * msg->points[i].z + msg->points[i].z);
//                if(dis > blind)
//                    multi_echo_num++;
//            }
        }
//        cout << "point num = " << multi_echo_num << endl;
    }
}
