...
void Preprocess::avia_handler(const livox_ros_driver::CustomMsg::ConstPtr &msg)
{
  ...
  if (feature_enabled)
  {
    ...
    {
      if ((msg->points[i].line < N_SCANS) && ((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00) && msg->points[i].x > 2.0)
      {
...
void Preprocess::rslidar_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  {
    int width_step = plsize / N_SCANS;
    // printf("%d/%d=%d\n", plsize, N_SCANS, width_step);
    for (int i = 0; i < plsize; i++)
    {
      PointType added_pt;
      // cout<<"!!!!!!"<<i<<" "<<plsize<<endl;
      ...
      added_pt.z = pl_orig.points[i].z;
      added_pt.intensity = pl_orig.points[i].intensity;
      added_pt.curvature = (float)(i % width_step) / width_step * 100.; // curvature unit: ms
...
void Preprocess::hesai_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  // ROS_INFO("%lf: %lf %lf ", msg->header.stamp.toSec(), pl_orig.points[0].timestamp, pl_orig.points[plsize - 1].timestamp);
  ...
  else
  {
    ...
    for (int i = 0; i < plsize; i++)
    {
      ...
      added_pt.z = pl_orig.points[i].z;
      added_pt.intensity = pl_orig.points[i].intensity;
      added_pt.curvature = (pl_orig.points[i].timestamp - begin_time) * 1000.0; // curvature unit: ms
